def Segementation(processSegment, memorySize, mem):
    ...
    entry = find(lambda object: object.get("segment_no") == int(segment_no, 2), segTab)
    print(f"Concerned Entry of Segment Table: {entry}")
    
    if entry['limit'] < int(segment_offset, 2):
        print("Invalid Address")

    physical_address = bin((entry["base_address"] + int(segment_offset, 2)))[2:]
...
if __name__ == "__main__":
    ...
    mem = [
        ...
        {'address': 2300, 'status': 'free'}, {'address': 2400, 'status': 'free'},
